...
def kalman_filter(z_meas, x_esti, P):
    ...
    x_pred = A1 @ x_esti + dt*u_esti
    P_pred = A1 @ P @ A1.T + Q
 
    # (2) Kalman Gain.
    K = P_pred @ H1.T @ np.linalg.inv(H1 @ P_pred @ H1.T + R)
 
    # (3) Estimation.
    x_esti = x_pred + K @ (z_meas - H1 @ x_pred)
 
    # (4) Error Covariance.
    P = P_pred - K @ H1 @ P_pred
...
while(1):
    ...
    r1 = random.uniform(-0.3, 0.3)
    r2 = random.uniform(-0.3, 0.3)
    r3 = random.uniform(-0.3, 0.3)
    rx[0][0] = x[0][0] + r1
    rx[1][0] = x[1][0] + r2
    rx[2][0] = x[2][0] + r3
    #rx: pos with noise
