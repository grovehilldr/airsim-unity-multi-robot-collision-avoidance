...
f1 = client.moveToPositionAsync(0, -10, -7, 2, vehicle_name="Drone1")
f2 = client.moveToPositionAsync(0, 7, -7, 2, vehicle_name="Drone2")
f3 = client.moveToPositionAsync(10, 7, -7, 2, vehicle_name="Drone3")
f4 = client.moveToPositionAsync(10, -10, -7, 2, vehicle_name="Drone4")
...
dgoal = np.array([ [8,8,2,2], [5,-1,-1,5] , [-6,-6,-6,-6] ])


def create_single_integrator_barrier_certificate(barrier_gain=.1, safety_radius=3, magnitude_limit=.5,boundary_points = np.array([-4,14,-7,11,-8.6,-4.4])):
    ...
    def f(dxi, x):
        ...
        N = dxi.shape[1]
        num_constraints = int(comb(N, 2)) + 6*N
        ...
        for k in range(N):
            ...
            count += 1
            #Pos Y
            A[count, (3*k,3*k+1, 3*k+2)] = np.array([0,1,0])
            ...
            #Pos X
            A[count, (3*k,3*k+1, 3*k+2)] = np.array([1,0,0])
